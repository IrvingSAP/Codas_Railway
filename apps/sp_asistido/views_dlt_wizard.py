"""Asistente DLT (DELETE / baja lógica) — pasos 1–6 (checklist D)."""
...
from apps.sp_asistido.models import SPCondition, SPDefinition, SPStepState
...
from apps.sp_asistido.services.generate_delete_script import (
    build_delete_procedure_sql,
    persist_generated_dlt_script,
)
...
SESSION_DLT_STEP1 = "sp_asistido_dlt_step1"


def _get_dlt_definition(request: HttpRequest, definition_id: int) -> SPDefinition:
...
def _dlt_where_conditions(definition: SPDefinition) -> list[SPCondition]:
    return list(
...
def _dlt_where_rows_for_step3(
    details: list[DetailTable], saved_by_field: dict
) -> list[dict]:
    """Una fila por columna (checkboxes + operador/origen/valor), igual criterio que UPD paso 4."""
    rows: list[dict] = []
    for d in details:
...
def _dlt_step4_payload(definition: SPDefinition) -> dict:
    st = SPStepState.objects.filter(
        sp_definition=definition, step_number=4
    ).first()
    if not st or not isinstance(st.payload_json, dict):
        return {"mode": "fisico", "logical_field": "", "logical_value": ""}
    p = st.payload_json
    return {
        "mode": (p.get("mode") or "fisico").strip().lower(),
        "logical_field": (p.get("logical_field") or "").strip(),
        "logical_value": (p.get("logical_value") or "").strip(),
    }
...
def dlt_wizard_step1(request: HttpRequest) -> HttpResponse:
    profile = request.user.profile

    initial, expired = load_wizard_session(request, SESSION_DLT_STEP1)
    initial = initial or {}
    if expired:
        notify_warning(
            request,
            "La sesión previa del asistente DLT expiró y se limpió automáticamente.",
        )
    errors: list[str] = []
    display = {
    ...
    if request.method == "POST":
        ...
        if not errors:
            ...
            return redirect("sp_asistido:dlt_step2")

    return render(
        request,
        "sp_asistido/dlt_wizard_step1.html",
        {
            "profile": profile,
...
@_require_sp_asistido_list_access
@require_http_methods(["GET", "POST"])
def dlt_wizard_step2(request: HttpRequest) -> HttpResponse:
    profile = request.user.profile
    data, expired = load_wizard_session(request, SESSION_DLT_STEP1)
    if expired:
        notify_warning(
            request,
            "La sesión del asistente DLT expiró. Inicie nuevamente desde el paso 1.",
        )
    ...
    if request.method == "POST":
        ...
        if not errors:
            ...
            if result.ok:
                ...
                clear_wizard_session(request, SESSION_DLT_STEP1)
                notify_success(request, result.error_message)
                return redirect(
                    "sp_asistido:dlt_step",
    ...
    return render(
        request,
        "sp_asistido/dlt_wizard_step2.html",
        {
            "profile": profile,
            "dashboard_nav_active": "sp_asistido",
            "current_step": 2,
...
def dlt_wizard_step_detail(
    ...
    definition = _get_dlt_definition(request, definition_id)

    if step == 3:
        return _dlt_step3(request, definition, profile)
    if step == 4:
        return _dlt_step4(request, definition, profile)
    if step == 5:
        return _dlt_step5(request, definition, profile)
    return _dlt_step6(request, definition, profile)
...
def _dlt_step3(
    ...
    wqs = _dlt_where_conditions(definition)
    ...
    if request.method == "POST":
        ...
        if not sel:
            errors.append(
                "Seleccione al menos un campo para el predicado WHERE (borrado).",
            )
        if not errors:
    ...
    if request.method == "POST" and errors:
        ...
        return render(
            request,
            "sp_asistido/dlt_wizard_step3.html",
            {
                "profile": profile,
    ...
    where_rows = _dlt_where_rows_for_step3(details, saved_by_field)
    return render(
        request,
        "sp_asistido/dlt_wizard_step3.html",
        {
...
def _dlt_step4(
    ...
    payload = _dlt_step4_payload(definition)
    errors: list[str] = []

    if request.method == "POST":
        mode = (request.POST.get("mode") or "fisico").strip().lower()
        lf = request.POST.get("logical_field", "")
        lv = request.POST.get("logical_value", "")
        errors = validate_dlt_mode_payload(
            mode,
            lf,
            lv,
            header_id=definition.header_table_id,
        ...
        if not errors:
            upsert_step_state(
                definition,
                4,
                {"mode": mode, "logical_field": lf.strip(), "logical_value": lv.strip()},
                request.user,
            )
    ...
    return render(
        request,
        "sp_asistido/dlt_wizard_step4.html",
        {
            "profile": profile,
            "definition": definition,
            "dashboard_nav_active": "sp_asistido",
            "current_step": 4,
            "mode": payload["mode"],
            "logical_field": payload["logical_field"],
            "logical_value": payload["logical_value"],
            "errors": errors,
        },
    )
...
def _dlt_step5(
    ...
    st4 = SPStepState.objects.filter(
        sp_definition=definition, step_number=4
    ).first()
    if not st4 or not isinstance(st4.payload_json, dict) or not st4.payload_json.get(
        "mode"
    ):
        notify_warning(request, "Indique modo físico o lógico antes de confirmar.")
        return redirect(
            "sp_asistido:dlt_step",
            definition_id=definition.pk,
            step=4,
    ...
    wcs = _dlt_where_conditions(definition)
    mode_payload = _dlt_step4_payload(definition)

    if request.method == "POST":
        confirm = request.POST.get("confirm_risk") == "1"
        phrase = (request.POST.get("confirm_phrase") or "").strip().upper()
        if not confirm or phrase != "ELIMINAR":
            errors = [
                "Debe marcar la casilla de riesgo y escribir exactamente ELIMINAR para continuar.",
            ]
            return render(
                request,
                "sp_asistido/dlt_wizard_step5.html",
                {
                    "profile": profile,
                    "definition": definition,
                    "dashboard_nav_active": "sp_asistido",
...
def _dlt_step6(
    ...
    if not wcs:
        notify_error(
            request,
            "No se puede generar DLT sin condición WHERE (regla D.8).",
        )
        return redirect(
            "sp_asistido:dlt_step",
            definition_id=definition.pk,
    ...
    mode_payload = _dlt_step4_payload(definition)
    mode_physical = mode_payload["mode"] != "logico"
    logical_detail = None
    if not mode_physical:
        logical_detail = DetailTable.objects.filter(
            header_id=definition.header_table_id,
            field_name_short__iexact=mode_payload["logical_field"],
            status=DetailTable.Status.ACTIVE,
        ).first()
        if not logical_detail:
            notify_error(request, "Campo de estado inválido para modo lógico.")
            return redirect(
                "sp_asistido:dlt_step",
                definition_id=definition.pk,
                step=4,
    ...
    preview_sql = build_delete_procedure_sql(
        definition,
        wcs,
        mode_physical=mode_physical,
        logical_status_detail=logical_detail,
        logical_value_raw=mode_payload["logical_value"],
    ...
    if request.method == "POST":
        ...
        if action == "confirm":
            result = confirm_generated_script(
                ...
                preview_sql,
                user=request.user,
                persist_fn=persist_generated_dlt_script,
                persist_kwargs={"where_conditions": wcs},
            )
            if result.ok:
                notify_success(request, result.error_message)
                return redirect("sp_asistido:list")
    ...
    return render(
        request,
        "sp_asistido/dlt_wizard_step6.html",
        {
            "profile": profile,
            "definition": definition,
            "dashboard_nav_active": "sp_asistido",
