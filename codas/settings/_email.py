...
# Modos admitidos en EMAIL_DELIVERY: smtp | console | auto
...
def _resolve_delivery_mode(*, require_smtp: bool) -> str:
    ...
    - EMAIL_DELIVERY=smtp|console: fuerza el modo.
    - EMAIL_DELIVERY=auto o vacío: en producción SMTP; en local SMTP si hay credenciales.
    """
    ...
    if raw not in (EMAIL_DELIVERY_AUTO, ""):
        raise ImproperlyConfigured(
            f"EMAIL_DELIVERY inválido: {raw!r}. Use smtp, console o auto."
        )
    if require_smtp:
        return EMAIL_DELIVERY_SMTP
...
def build_email_settings(*, require_smtp: bool) -> dict[str, Any]:
    ...
    default_from = (os.environ.get("DEFAULT_FROM_EMAIL") or "").strip() or host_user or "noreply@codas.local"
    ...
    if require_smtp and not _smtp_credentials_configured():
        raise ImproperlyConfigured(
            "Correo SMTP obligatorio en producción: defina EMAIL_HOST_USER y "
            "EMAIL_HOST_PASSWORD en .env (Gmail: contraseña de aplicación)."
        )
...
def validate_email_settings_for_production() -> None:
    ...
    build_email_settings() según EMAIL_DELIVERY y las credenciales SMTP.
    """
    if (os.environ.get("EMAIL_BACKEND") or "").strip():
        raise ImproperlyConfigured(
            "No defina EMAIL_BACKEND en producción. Use EMAIL_DELIVERY=smtp y "
            "EMAIL_HOST_USER / EMAIL_HOST_PASSWORD (u otras EMAIL_*). "
